...
extern "C" {
	int launch_tls_client(struct tls_control_args* config, char* server_name, char* server_port);
...
static int cert_verify_callback(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags) 
{
	...
	if (g_control_config.fail_cert_verify_callback ||
	    g_control_config.fail_enclave_identity_verifier_callback ||
		g_control_config.fail_oe_verify_tls_cert)
	{
		mbedtls_printf("Purposely returns failure from client's cert_verify_callback()\n");
	...
	mbedtls_printf("returned from oe_verify_tls_cert\n");
	ret = 0;
	*flags = 0;
exit:
	return ret;
...
int configure_client_ssl(mbedtls_ssl_context *ssl,
						...
						mbedtls_pk_context *private_key)
{
	...
	// MBEDTLS_SSL_VERIFY_OPTIONAL: peer certificate is checked, however the handshake continues
	//                              even if verification failed; mbedtls_ssl_get_verify_result()
	//                              can be called after the handshake is complete.
	...
	//            one cert/key pair (eg one ECDSA, one RSA with SHA-256, one RSA with
	//            SHA-1). An adequate certificate will be selected according to the
	//            client's advertised capabilities. In case multiple certificates are
	//            adequate, preference is given to the one set by the first call to this
	//            function, then second, etc.
	// On client, only the first call has any effect. That is, only one client certificate
	//            can be provisioned. The server's preferences in its CertficateRequest
	//            message will be ignored and our only cert will be sent regardless of whether
	//            it matches those preferences - the server can then decide what it wants to do with it.
	//
	// The provided pk_key needs to match the public key in the first certificate in own_cert,
	// or all handshakes using that certificate will fail. It is your responsibility to ensure
	// that; this function will not perform any check. You may use mbedtls_pk_check_pair() in
	// order to perform this check yourself, but be aware that this function can be computationally
	// expensive on some key types.
	...
	{
		mbedtls_printf("failed! mbedtls_ssl_setup returned %d\n", ret);
		goto exit;
	}
	ret = 0;
exit:
...
int launch_tls_client(struct tls_control_args* config, char* server_name, char* server_port)
{
	...
	mbedtls_printf("(E)client: fail_cert_verify_callback=%d\n", config->fail_cert_verify_callback);
	mbedtls_printf("(E)client: fail_enclave_identity_verifier_callback=%d\n", config->fail_enclave_identity_verifier_callback);
	mbedtls_printf("(E)client: fail_oe_verify_tls_cert=%d", config->fail_oe_verify_tls_cert);

	g_control_config = *config;

	if (CLIENT_REQUEST_PAYLOAD_SIZE != strlen(GET_REQUEST))
	{
		mbedtls_printf("Error: this client's request payload size does not match"
	...
	//
	mbedtls_printf("(E)Connecting to tcp/%s/%s...\n", server_name, server_port);
	...
	mbedtls_printf("Connected to server @%s.%s\n", server_name, server_port);
	...
	{ 
		mbedtls_printf( "Failed! mbedtls_ssl_set_hostname returned %d\n", ret );
		goto exit;
	}
	...
	if (uret != 0)
	{
		mbedtls_printf("client mbedtls_ssl_handshake failed with uret = 0x%x\n", uret);
		mbedtls_ssl_close_notify(&ssl);
		exit_code = MBEDTLS_EXIT_FAILURE;
		goto exit;
	}
	mbedtls_printf("client mbedtls_ssl_handshake ok\n");
	...
	do
	{
		...
		{
			mbedtls_printf("ERROR: expected reading %d bytes but only got %d bytes\n", SERVER_RESPONSE_PAYLOAD_SIZE, len);
			exit_code = MBEDTLS_EXIT_FAILURE;
			goto exit;
		}
		else
		{
			mbedtls_printf("Client done reading server data\n");
			break;
		}
	} while (1);
...
int setup_tls_server(struct tls_control_args* config, char* server_port)
{
	(void)config;
	(void)server_port;
	mbedtls_printf("Client: calling setup_tls_server: Never reach here\n");
	return 0;
}
