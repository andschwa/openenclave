...
static int cert_verify_callback(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags) 
{
	...
	{
		mbedtls_printf("Purposely returns failure from server's cert_verify_callback()\n");
...
int setup_tls_server(struct tls_control_args *config, char* server_port)
{
	...
	mbedtls_printf("(E)server: fail_cert_verify_callback=%d\n", config->fail_cert_verify_callback);
	mbedtls_printf("(E)server: fail_enclave_identity_verifier_callback=%d\n", config->fail_enclave_identity_verifier_callback);
	mbedtls_printf("(E)server: fail_oe_verify_tls_cert=%d\n", config->fail_oe_verify_tls_cert);
...
waiting_for_connection_request:
	...
	if (uret != 0)
	{
		mbedtls_printf("server mbedtls_ssl_handshake failed with uret = 0x%x\n", uret);
		mbedtls_ssl_close_notify(&ssl);
		ret = MBEDTLS_EXIT_FAILURE;
		goto exit;
	}

	mbedtls_printf("server mbedtls_ssl_handshake done successfully\n");
